...
def test_output():
    ...
    assert 0 == main(['-i', './examples/Test.fidl', '-o', './examples/Test2.adoc'])
...
def test_wrong_args(capsys):
    assert 1 == main(['-a', './examples/Test.fidl', '-o', './examples/Test2.adoc'])
    captured = capsys.readouterr()
    assert captured.out.startswith('fidl2adoc.py -i <inputfile> [-i <inputfile2>]* -o <outputfile>')
